...
from .parts import DoubleConvBlock, UNetDsv2d, UNetUp, Conv2dBatchNormReLU


class UNet2dFusedAttentionDsvMultiscale(nn.Module):
    ...
    def __init__(self, feature_scale=4, n_classes=1, is_deconv=False, in_channels=1,
        ...
        super(UNet2dFusedAttentionDsvMultiscale, self).__init__()
        ...
        self.inputs_scale2 = Conv2dBatchNormReLU(in_channels=self.in_channels * 2, out_channels=filters[0] * 2//encoder_scale,
                                                   kernel_size=(3, 3), padding=(1, 1))
        self.inputs_scale3 = Conv2dBatchNormReLU(in_channels=self.in_channels * 2, out_channels=filters[1] * 2//encoder_scale,
                                                   kernel_size=(3, 3), padding=(1, 1))
        self.inputs_scale4 = Conv2dBatchNormReLU(in_channels=self.in_channels * 2, out_channels=filters[2] * 2//encoder_scale,
                                                   kernel_size=(3, 3), padding=(1, 1))

        # downsampling
        self.conv1 = DoubleConvBlock(self.in_channels, filters[0] // encoder_scale, self.is_batchnorm,
                                     kernel_size=(3, 3), padding=(1, 1))
        self.maxpool1 = nn.MaxPool2d(kernel_size=(2, 2))

        self.conv2 = DoubleConvBlock(filters[0] // encoder_scale * 3, filters[1] // encoder_scale, self.is_batchnorm,
                                     kernel_size=(3, 3), padding=(1, 1))
        self.maxpool2 = nn.MaxPool2d(kernel_size=(2, 2))

        self.conv3 = DoubleConvBlock(filters[1] // encoder_scale * 3, filters[2] // encoder_scale, self.is_batchnorm,
                                     kernel_size=(3, 3), padding=(1, 1))
        self.maxpool3 = nn.MaxPool2d(kernel_size=(2, 2))

        self.conv4 = DoubleConvBlock(filters[2] // encoder_scale * 3, filters[3] // encoder_scale, self.is_batchnorm,
                                     kernel_size=(3, 3), padding=(1, 1))
        self.maxpool4 = nn.MaxPool2d(kernel_size=(2, 2))

        self.center = DoubleConvBlock(filters[3] // encoder_scale, filters[4] // encoder_scale, self.is_batchnorm,
                                      kernel_size=(3, 3), padding=(1, 1))

        # downsampling (2nd branch)
        self.conv1_2 = DoubleConvBlock(self.in_channels, filters[0] // encoder_scale, self.is_batchnorm,
                                       kernel_size=(3, 3), padding=(1, 1))
        self.maxpool1_2 = nn.MaxPool2d(kernel_size=(2, 2))

        self.conv2_2 = DoubleConvBlock(filters[0] // encoder_scale * 3, filters[1] // encoder_scale, self.is_batchnorm,
                                       kernel_size=(3, 3), padding=(1, 1))
        self.maxpool2_2 = nn.MaxPool2d(kernel_size=(2, 2))

        self.conv3_2 = DoubleConvBlock(filters[1] // encoder_scale * 3, filters[2] // encoder_scale, self.is_batchnorm,
                                       kernel_size=(3, 3), padding=(1, 1))
        self.maxpool3_2 = nn.MaxPool2d(kernel_size=(2, 2))

        self.conv4_2 = DoubleConvBlock(filters[2] // encoder_scale * 3, filters[3] // encoder_scale, self.is_batchnorm,
                                       kernel_size=(3, 3), padding=(1, 1))
        self.maxpool4_2 = nn.MaxPool2d(kernel_size=(2, 2))

        self.center_2 = DoubleConvBlock(filters[3] // encoder_scale, filters[4] // encoder_scale, self.is_batchnorm,
                                        kernel_size=(3, 3), padding=(1, 1))

        # ====

        # gating signal
        self.gating4 = UnetGridGatingSignal2(filters[4] * 2//encoder_scale, filters[4], kernel_size=(1, 1),
                                             is_batchnorm=self.is_batchnorm)

        # attention blocks
        self.fusedskip1 = Conv2dBatchNormReLU(in_channels=filters[0] * 2 // encoder_scale, out_channels=filters[0],
                                              kernel_size=(3, 3), stride=1, padding=1)
        self.fusedattention2 = FusedAttentionBlock(in_channels=filters[1]//encoder_scale, gate_channels=filters[2],
                                                   inter_channels=filters[1]//encoder_scale,
                                                   nonlocal_mode=nonlocal_mode, sub_sample_factor=attention_dsample)
        self.fusedattention3 = FusedAttentionBlock(in_channels=filters[2]//encoder_scale, gate_channels=filters[3],
                                                   inter_channels=filters[2]//encoder_scale,
                                                   nonlocal_mode=nonlocal_mode, sub_sample_factor=attention_dsample)
        self.fusedattention4 = FusedAttentionBlock(in_channels=filters[3]//encoder_scale, gate_channels=filters[4],
                                                   inter_channels=filters[3]//encoder_scale,
                                                   nonlocal_mode=nonlocal_mode, sub_sample_factor=attention_dsample)
